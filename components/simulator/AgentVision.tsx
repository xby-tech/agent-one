...
export default function AgentVision({ history, currentSignals, currentLabel, humanInstruction }: AgentVisionProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#F5F5F7] p-4 md:p-6 overflow-y-auto">
      ...
      {/* History entries — older ones collapsible, most recent stays expanded */}
      {history.map((entry, i) => {
        const isLatest = i === history.length - 1 && currentSignals.length > 0;
        const isOldest = i < history.length - 1;
        const isExpanded = isOldest ? expandedIndexes.has(i) : true;
          ...
          <div key={entry.stepId} className="mb-2">
            ...
            {isExpanded && (
              <motion.div
                ...
                className={`space-y-1 pb-2 ${isOldest ? 'pl-4 opacity-70' : ''}`}
              >
