...
export default function AgentVision({ history, currentStepId, currentSignals, currentLabel, humanInstruction }: AgentVisionProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#F5F5F7] p-4 md:p-6 overflow-y-auto">
      ...
      {/* History entries — current step open, rest collapsed */}
      {history.map((entry, i) => {
        const isCurrent = entry.stepId === currentStepId;
        const isExpanded = isCurrent || expandedIndexes.has(i);
          ...
          <div key={entry.stepId} className="mb-2">
            ...
            {isExpanded && (
              <motion.div
                ...
                className={`space-y-1 pb-2 ${isCurrent ? '' : 'pl-4 opacity-70'}`}
              >
                {entry.signals.map((signal, si) => (
                  <DataSignal key={`${entry.stepId}-${si}`} signal={signal} index={isCurrent ? si : 0} />
                ))}
              </motion.div>
            )}
          </div>
